...
public class BSplayerSwap : BSstate
{
    ...
    private void HandleMessage (string id, FormattedMessage fMsg)
    {
        if (id.Equals("UI"))
        {
            if (Core.CoreManager.Instance.worldStateManager.State == Core.WorldState.Battle)
            {
                ...
                else if(fMsg.key == "Party")
                {
                    PartyMessageObject message = JsonUtility.FromJson<PartyMessageObject>(fMsg.message);
                    currentSelectedIndex = message.index;
                }
            }
        }
    }
