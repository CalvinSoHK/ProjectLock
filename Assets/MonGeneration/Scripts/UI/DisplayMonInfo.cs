...
public class DisplayMonInfo : MonoBehaviour
{
    ...
    private void Update()
    {
        if (generator.CheckValidID(currentID))
        {
            displayMon = generator.GetMonByID(currentID);
            CheckButtonActive();
            ...
            evolution.text = generator.CheckValidID(displayMon.next_evoID) ? 
                generator.GetMonByID(displayMon.next_evoID).name + 
                " " + generator.GetMonByID(displayMon.next_evoID).ID : "";
            previous.text = generator.CheckValidID(displayMon.prev_evoID) ?
                generator.GetMonByID(displayMon.prev_evoID).name +
                " " + generator.GetMonByID(displayMon.prev_evoID).ID : "";
    ...
    {
        if (!generator.CheckValidID(currentID + 1))
        {
        ...
        if (currentID - 1 <= 0 || !generator.CheckValidID(currentID - 1))
        {
