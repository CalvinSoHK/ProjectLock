...
namespace World.Condition
{
    ...
    public class ItemCheckCondition : BaseCondition
    {
        ...
        {
            bool hasItem = Core.CoreManager.Instance.playerInventory.Inventory.HasItem(item.ItemName);
            if(itemCount == 1)
            {
                OnCondition?.Invoke(hasItem, ConditionID);
            }
            else if (hasItem && itemCount <= Core.CoreManager.Instance.playerInventory.Inventory.GetItemCount(item.ItemName))
            {
